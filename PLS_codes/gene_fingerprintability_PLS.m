...
nspins = 1000; 
for k = 1:nspins    
    YSPIN = Y(spins(:,k),:);  % permute neurosynth matrix
    [nullrtrain,nullrtest] = fcn_crossval_pls_brain_obvs(exp,YSPIN,100,0.75,1,coordsS200);
    nullrtrain_all(k)=median(nullrtrain);
    nullrtest_all(k)=median(nullrtest);
end

sum(nullrtest_all > median(rtest))/nspins

save('./cross_validation_rtest.mat','rtest', '-v7.3') 

save('./cross_validation_nulls.mat','nullrtest_all', '-v7.3') 

% p value after cross validation 0.0020

% visualize boxplots as distributions of the correlations computed using
% the training and testing set

plot_data=[rtrain rtest];
plot_data(101:1000,1:2)= NaN
...
figure;
boxplot([ plot_data nullrtest_all'])
set(gca,'xticklabel',{'train','test','null'})
ylabel('correlation')
title('cross validation')
ylim([-0.9, 0.9])
...
%% distribute PLS-derived gene and ICC scores
