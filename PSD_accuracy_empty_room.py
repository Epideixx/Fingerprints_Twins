...
FOLDER_RESULTS = os.path.join(FOLDER_RESULTS, "PSD_Accuracy_empty_room")
if not os.path.exists(FOLDER_RESULTS):
    os.mkdir(FOLDER_RESULTS)
...
record_empty = pd.read_csv(os.path.join(DATA_PATH, "record_Empty_Room_HCP_Schaefer.csv"), index_col="Subject_ID")
...
record_empty = np.log(record_empty)
...
for i in range(1, 5):
    for j in range(1, 5):
        if i == 4 or j == 4 :
            ...
            records = {1 : record_1, 2 : record_2, 3 : record_3, 4 : record_empty}
            ...
            folder_result = os.path.join(FOLDER_RESULTS, folder_result)
...
save_acc_path = os.path.join(FOLDER_RESULTS, "All_accuracies_every_freq_stacked.csv")
